...
def sendCommandToArduino():
    ...
    #tem=chr(0x01)+chr(currentDirection)+chr(0xF0)+chr(checkSum)
    ser.write(chr(0x01)+chr(currentDirection)+chr(0xF0)+chr(checkSum))
    #ser.write(tem.encode())
    print("sent")

def sendThreeSixty():
    if(threeSixty):
        checkSum = 0x02+0x01+0xF0
        #ser.write(chr(0x02).encode()+chr(0x01).encode()+chr(0xF0).encode()+chr(checkSum).encode())
        ser.write(chr(0x02)+chr(0x01)+chr(0xF0)+chr(checkSum))
        print("sent 360")
    else:
        checkSum = 0x02+0x00+0xF0
        #ser.write(chr(0x02).encode()+chr(0x00).encode()+chr(0xF0).encode()+chr(checkSum).encode())
