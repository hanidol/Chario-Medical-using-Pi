...
#ACCESS_TOKEN = 'HKIKKxS2VNDK747Lvwtb'
THINGSBOARD_HOST = '192.168.111.132'

ACCESS_TOKEN = '1eC6o4SPyzzOCrQgbhEI'


sensor_data = {'Body Tempurature': 0, 'Ambient Tempurature': 0,'used_space': 0, 'cpu_load': 0, 'Voltage' : 0, 'swap' : 0, 'memory': 0, 'cpu_temp' :0, 'model_name' :0, 'uptime' : 0}
...
def connect():
    ...
    #client = mqtt.Client(cpu_load,cpu_temp, used_space,voltage,sys_clock_speed, swap, memory, uptime)
    ...
    #print("used_space: %-3.1f C" % used_space)

    #print("cpu_load: %-3.1f %%" % cpu_load)

    #print("Voltage: %-3.1f C" % voltage)

    #print("swap: %-3.1f %%" % swap)

    #print("cpu_load: %-3.1f %%" % memory)

    #print("used_space: %-3.1f C" % cpu_temp)

    #print("cpu_load: %-3.1f %%" % str(model_name))

    #print("uptime: %-3.1f C" % uptime)

    sensor_data[' used_space'] =float(used_space)

    sensor_data['cpu_load'] = cpu_load

    sensor_data['Voltage'] = float(voltage) * 3 

    sensor_data['swap'] = swap

    sensor_data['memory'] = memory

    sensor_data['cpu_temp'] = float(cpu_temp)

    sensor_data[' model_name'] =  model_name

    sensor_data['uptime'] =uptime

    sensor_data['sys_clock_speed'] = int(sys_clock_speed)
